...
class Admin extends Admin_Controller {
    ...
    protected $work_categories = array();
    ...
    protected $validation_rules = array(
        ...
        'id_work_categories' => array(
            'field' => 'id_work_category',
    ...
    public function __construct() {
        ...
        $this->load->model(array('blog_m', 'blog_categories_m', 'excel_m'));
        $this->lang->load(array('blog', 'categories'));
        $this->load->model('contents/work_companies_m');
        ...
        //Companies
        $this->companies = $this->work_companies_m->convert_to_select();
        $this->work_categories = $this->work_categories_m->get_valkeys();
        ...
        $this->template
                ...
                ->set('companies', $this->companies)
                ->set('work_categories', $this->work_categories)
    ...
    public function index() {
        ...
        $this->blog_m->where('status_crawler', 1);
        ...
        if ($this->input->post('f_company')) {
            $this->blog_m->where('id_company', (int) $this->input->post('f_company'));
        }
        ...
        if ($this->input->post('id_work_category')) {
            $id = $this->input->post('id_work_category');
            $this->db->like('id_work_categories', $id);
        ...
        if ($this->input->post('btnAction')) {
            $params = array();
            $all_blog = $this->blog_m
                    ->get_many_by($base_where);
            if (!empty($all_blog)) {
                foreach ($all_blog as $key => $item) {

                    //get Company
                    $company = $this->blog_m->get_company_by_id($item->id_company);
                    $params[$key] = array();
                    $params[$key]['#'] = $key + 1;
                    $params[$key]['Title'] = $item->title;
                    $params[$key]['Description'] = $item->description;
                    $params[$key]['Requirements'] = $item->requirements;
                    $params[$key]['Expired'] = format_date($item->end);
                    $params[$key]['Company Name'] = $company[0]->name;
                    $params[$key]['Company Website'] = $company[0]->website;
                }
                $array_info = array(
                    'creator' => 'Admin',
                    'lasmodifiedby' => 'demo',
                    'title' => 'Jobs',
                    'subject' => 'Jobs',
                    'description' => 'Statistic all jobs',
                    'keywords' => 'jobs',
                    'category' => 'jobs',
                );
                $id_category = $this->input->post('id_work_category');
                $work_categories_keys = $this->work_categories_m->get_valkeys_by_id();
                
                $filename = date('d-m-Y');
                $this->excel_m->set_Properties_Excel($array_info);
                $this->excel_m->push_Data_Excel($params, $id_category != 0 ? $work_categories_keys[$id_category] : 'All');
                $this->excel_m->export_to_Excel_Download($filename);
            }
            else {
                $this->session->set_flashdata('notice', 'No item to export');
            }
        }
    ...
    public function create() {
        ...
        if ($this->form_validation->run()) {
            ...
            $extra = array(
                ...
                'id_work_categories' => implode(',', $this->input->post('id_work_category')),
                'status_crawler' => 1
            );
